...
export async function resetHomingOffsets(
  ...
  for (let i = 0; i < motorIds.length; i++) {
    ...
    try {
      ...
      throw new Error(`Failed to reset homing offset for motor ${motorId}`, { cause: error });
    }
  }
}
...
export async function writeHomingOffsetsToMotors(
  ...
  for (let i = 0; i < motorIds.length; i++) {
    ...
    try {
      ...
      throw new Error(`Failed to write homing offset for ${motorName}`, { cause: error });
    }
  }
}
...
export async function writeHardwarePositionLimits(
  ...
  for (let i = 0; i < motorIds.length; i++) {
    ...
    try {
      ...
      throw new Error(`Failed to write position limits for ${motorName}`, { cause: error });
    }
  }
}

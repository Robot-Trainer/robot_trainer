...
export class BiSoFollowerConfig extends RobotConfig {
  get type(): string {
    return "bi_so_follower";
  }
}

export class BiSoFollower extends Robot {
  constructor(config: BiSoFollowerConfig) {
    super(config);
  }

  async connect(_calibrate?: boolean): Promise<void> {
    // Implementation specific to bi_so_follower
  }

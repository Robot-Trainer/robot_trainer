...
export class BiOpenarmFollowerConfig extends RobotConfig {
  get type(): string {
    return "bi_openarm_follower";
  }
}

export class BiOpenarmFollower extends Robot {
  constructor(config: BiOpenarmFollowerConfig) {
    super(config);
  }

  async connect(_calibrate?: boolean): Promise<void> {
    // Implementation specific to bi_openarm_follower
  }

import { WebTeleoperator } from "./teleoperators/base-teleoperator.js";
import { MotorConfig } from "./types/teleoperation.js";
import * as parquet from "parquet-wasm";
import * as arrow from "apache-arrow";
import JSZip from "jszip";
import generateREADME from "./utils/record/generateREADME.js";
...
type LeRobotAction = number[] | Record<string, number>;
type InterpolatableValue = LeRobotAction;

interface RecordedEpisodeFrame {
...
interface DatasetFeatureInfo {
  "video.fps": number;
  "video.height": number;
  "video.width": number;
  "video.channels": number;
  "video.codec": string;
...
export interface LeRobotExportBlob {
  path: string;
  content: Blob;
}

export type LeRobotExportResult = LeRobotExportBlob[] | Blob;
...
function toActionRecord(action: LeRobotAction): Record<string, number> {
  if (Array.isArray(action)) {
    const record: Record<string, number> = {};
    JOINT_ORDER.forEach((joint, index) => {
      record[joint] = action[index] ?? 0;
    });
    return record;
  }
...
export class LeRobotEpisode {
  ...
  private static interpolateArrays(
    array1: InterpolatableValue,
    array2: InterpolatableValue,
    factor: number
  ): InterpolatableValue {
    ...
    } else if (isNumericRecord(array1) && isNumericRecord(array2)) {
    ...
    throw new Error("Interpolation supports only numeric arrays or numeric records");
  }
}
...
export class LeRobotDatasetRecorder {
  ...
  private static getSupportedRecorderType(): { mime: string; ext: string } {
    ...
    for (const c of candidates) {
      ...
      if (
        mediaRecorderCtor &&
        mediaRecorderCtor.isTypeSupported &&
        mediaRecorderCtor.isTypeSupported(c.mime)
      ) {
        return c;
      }
    }
  ...
  get episodes(): LeRobotEpisode[] {
    ...
    for (let i = 0; i < this.teleoperatorData.length; i++) {
      const episode = this.teleoperatorData[i];
        ...
        return new Promise<void>((resolve) => {
          ...
          recorder.onstop = () => {
            ...
            if (!this.videoBlobsByEpisode[segmentEpisodeIndex]) {
              this.videoBlobsByEpisode[segmentEpisodeIndex] = {};
            }
        ...
        return new Promise<void>((resolve) => {
          ...
          recorder.onstop = () => {
            ...
            if (!this.videoBlobsByEpisode[oldSegmentEpisodeIndex]) {
              this.videoBlobsByEpisode[oldSegmentEpisodeIndex] = {};
            }
  ...
  _actionInterpolatate(
    action1: LeRobotAction,
    action2: LeRobotAction,
    timestamp1: number,
    timestamp2: number,
    reqTimestamp: number
    ...
    const action1Record = toActionRecord(action1);
    const action2Record = toActionRecord(action2);
    const actionKeys = Object.keys(action1Record);
    const interpolatedAction: Record<string, number> = {};
    ...
    for (let i = 0; i < actionKeys.length; i++) {
      ...
      interpolatedAction[key] =
        action1Record[key] +
        ((action2Record[key] - action1Record[key]) * (reqTimestamp - timestamp1)) /
          timeRange;
    }
   ...
   */
  convertActionToArray(action: LeRobotAction): number[] {
    if (Array.isArray(action)) {
      return JOINT_ORDER.map((_, index) => action[index] ?? 0);
    }
   ...
   */
  _findClosestTimestampBefore(
    data: NonIndexedLeRobotDatasetRow[],
    targetTimestamp: number
  ): number {
    let firstIndex = 0;
  ...
  _convertToLeRobotDataFormatFrames(
    episodeRough: RecordedEpisodeFrame[]
  ): NonIndexedLeRobotDatasetRow[] {
    ...
    for (let i = 0; i < episodeRough.length; i++) {
      ...
      properFormatFrames.push({
        ...
        action: this.convertMotorConfigToArray(
          frameRough.previousMotorConfigsNormalized
        ),
        "observation.state": this.convertMotorConfigToArray(
          frameRough.newMotorConfigsNormalized
        ),
  ...
  private async _exportEpisodesToBlob(
    ...
    const episodeBlobs: { content: Blob; path: string }[] = [];

    for (let i = 0; i < episodes.length; i++) {
      const episode = episodes[i];
      const data = episode.frames;
      const { tableFromArrays, vectorFromArray } = arrow;

      const timestamps = data.map((row) => row.timestamp);
      const actions = data.map((row) => row.action);
      const observationStates = data.map((row) => row["observation.state"]);
      const episodeIndexes = data.map((row) => row.episode_index);
      const taskIndexes = data.map((row) => row.task_index);
      const frameIndexes = data.map((row) => row.frame_index);
      const indexes = data.map((row) => row.index);

      const table = tableFromArrays({
        timestamp: timestamps,
        // @ts-expect-error, this works, idk why
   ...
   */
  exportEpisodes(format: "json"): LeRobotEpisode[];
  exportEpisodes(format: "blob"): Promise<{ content: Blob; path: string }[]>;
  exportEpisodes(
    format: "json" | "blob" = "json"
  ): LeRobotEpisode[] | Promise<{ content: Blob; path: string }[]> {
    if (this._isRecording)
  ...
  async generateMetadata(
    _data: NonIndexedLeRobotDatasetRow[]
   ...
   * @returns A Uint8Array blob containing the parquet data
   */
  async getEpisodeStatistics(_data: LeRobotEpisode[]): Promise<Uint8Array> {
  ...
  async _exportForLeRobotBlobs() {
    const regularizedEpisodes = this.exportEpisodes("json");
    ...
    const allEpisodeIndices = regularizedEpisodes.map((_, i) => i);
    ...
    const totalFrames = regularizedEpisodes.reduce(
      (sum: number, ep: LeRobotEpisode) => sum + ep.frames.length,
      0
    );
    ...
    const infoJson: DatasetInfoJson = {
      version: "2.1",
    ...
    // stats.json (minimal)
    const lengths = regularizedEpisodes.map((ep) => ep.frames.length);
    ...
    for (let epIdx = 0; epIdx < numEpisodes; epIdx++) {
      ...
      const timestamps = (regularizedEpisodes[epIdx]?.frames || []).map(
        (f: NonIndexedLeRobotDatasetRow) => f.timestamp
      );
...
export async function record(config: RecordConfig): Promise<RecordProcess> {
  ...
  if (config.robotType) {
    recorder.setRobotLabel(config.robotType);
  }
  ...
  const recordProcess: RecordProcess = {
    ...
    async nextEpisode(): Promise<number> {
      return recorder.nextEpisodeSegment();
    },

    restoreEpisodes(episodes: LeRobotEpisode[]): void {
      recorder.teleoperatorData = [...episodes];
    },

    addCamera(name: string, stream: MediaStream): void {
      recorder.addVideoStream(name, stream);
    },

    removeCamera(name: string): void {
      delete recorder.videoStreams[name];
    },

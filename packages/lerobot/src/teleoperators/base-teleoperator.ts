...
export interface RecordedMotorPositionFrame extends StateUpdateCallbackParams {
  episodeIndex: number;
  taskIndex: number;
}

type ClosableMotorCommunicationPort = MotorCommunicationPort & {
  close(): Promise<void>;
};

function isClosableMotorPort(
  port: MotorCommunicationPort
): port is ClosableMotorCommunicationPort {
  return (
    "close" in port &&
...
export abstract class WebTeleoperator {
  ...
  abstract get isActiveTeleoperator(): boolean;
...
export abstract class BaseWebTeleoperator extends WebTeleoperator {
  ...
  public recordedMotorPositionEpisodes : RecordedMotorPositionFrame[][];
  ...
  async disconnect(): Promise<void> {
    this.stop();
    if (isClosableMotorPort(this.port)) {

...
async function createTeleoperator(
  ...
  switch (config.teleop.type) {
    ...
    default: {
      // Exhaustiveness fallback for future teleoperator variants.
      const unsupportedType = (config.teleop as { type?: string }).type;
      throw new Error(
        `Unsupported teleoperator type: ${unsupportedType ?? "unknown"}`
      );
    }
  }
}
...
function buildTeleoperationStateFromTeleoperator(
  ...
  const isActive = teleoperator.isActiveTeleoperator;

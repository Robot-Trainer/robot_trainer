...
import { Advanced, Robot, Dataset, RobotConfiguration, Settings, Loader, Layout } from './icons';
...
const InnerApp: React.FC<{ externalLoading?: boolean }> = ({ externalLoading = false }) => {
  ...
  useEffect(() => {
    if (['robot-models', 'system-settings'].includes(activeTab)) setAdvancedOpen(true);
  }, [activeTab]);
          ...
          <div className="mb-8">
            ...
            <div>
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-colors mb-1 ${advancedOpen || ['robot-models', 'system-settings'].includes(activeTab) ? 'bg-gray-100 text-gray-800' : 'text-gray-600 hover:bg-gray-100'}`}>
                <Advanced className="h-4 w-4" />
                <div className="">
                  <span>Advanced</span>
                </div>
                <svg className={`h-4 w-4 transform transition-transform ${advancedOpen ? 'rotate-180' : ''}`} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">

...
import { robotModelsTable, usbVendorsTable, scenesTable } from "./schema";
import vendorIdsCsv from "../../../usb_vendors_and_products/vendor_ids.csv?raw";

function parseUsbVendorsCsv(csvText: string): (typeof usbVendorsTable.$inferInsert)[] {
  const lines = csvText.split(/\r?\n/);
  const rows: (typeof usbVendorsTable.$inferInsert)[] = [];
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const vendorId = Number.parseInt(match[2], 10);
    if (!company || !Number.isFinite(vendorId)) {
      continue;
    }

    rows.push({ company, vendorId });
  }
...
export async function seedRobotModels() {
  ...
  try {
    const usbVendorsData = parseUsbVendorsCsv(vendorIdsCsv);
    ...
    if (usbVendorsData.length > 0) {
      await db.insert(usbVendorsTable).values(usbVendorsData).onConflictDoNothing();
    }
    ...
    await db.execute(sql`SELECT setval(pg_get_serial_sequence('usb_vendors', 'id'), (SELECT MAX(id) FROM usb_vendors))`);
